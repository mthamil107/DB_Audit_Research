...
Prior work on defeating database audit consistently assumes an administrative adversary.
MemTraceDB~\cite{memtracedb} states its attacker ``has gained privileged access\ldots{} a
malicious insider or an external threat actor who has escalated their credentials to an
administrative level,'' and blinds by disabling logging or editing log files. We are not
aware of prior work evaluating reversible blinding of an unaudited trigger-based auditor by
a low-privilege, injection-only role.
...
(hash-chaining or signatures)~\cite{schneier1998,rfc6962}.
...
\midrule
1A plain catalog shadow & No & \code{pg\_catalog} resolved first; app cannot \code{CREATE SCHEMA} without \code{CREATE}-on-db \\
1B exact composite overload & \textbf{Yes} & writable \code{public} \emph{and} unpinned trigger \code{search\_path} \\
1C replace trigger fn body & \textbf{Yes} & app owns / can \code{ALTER} the audit fn \\
1D bespoke secdef helper & N/A & none beyond the built-ins of 1A/1B \\
\bottomrule
\end{tabularx}
\end{table}
...
1B is reachable on defaults; on stock 15+/16 \code{PUBLIC} has only \code{USAGE}, so 1B
needs an explicit re-grant or another writable schema on the function's path. Plain catalog
shadowing (1A) never blinds---the exact-overload specificity in 1B is what beats
\code{pg\_catalog}'s search-order priority.
...
identical before and after, so a source/object diff finds nothing. A naive
semantically-equivalent restore does \emph{not} match. What still betrays a perfect restore
is \emph{independent} of the audit code: a write-versus-audit canary shows a gap of one, and
a DDL event trigger recorded two \code{CREATE OR REPLACE FUNCTION} events. Object-diffing the
...
\paragraph{Honest scoping.} The strongest ``works on stock defaults'' claim holds only for
PostgreSQL~$\leq$14; on 15+ the \code{public}-schema default change makes 1B
misconfiguration-gated. We state the version and \code{public}-schema ACL for every result.
